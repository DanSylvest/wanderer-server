...
const subscriber = async function (_connectionId, _responseId, _event) {
    ...
    try {
        ...
        core.charactersController.get(_event.characterId).get("location").subscribe(_connectionId, _responseId);
    } catch (err) {
        helpers.errResponse(_connectionId, _responseId, responseName, "Error in subscribe location", {code: 1, handledError: err});
    }
};

subscriber.unsubscribe = function (_connectionId, _responseId, _event) {
    core.charactersController.get(_event.characterId).get("location").unsubscribe(_connectionId, _responseId);
};
