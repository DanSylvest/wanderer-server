...
class Map extends Emitter {
  ...
  deinit() {
    for (const systemId in this._systems) {
      this._systems[systemId].destructor();
    }

    for (const key in this._chains) {
  ...
  connectionBreak(_connectionId) {
    ...
    for (const solarSystemId in this._systems)
  ...
  _startClearTimeout() {
    this._timeoutId = setTimeout(
      ...
      SYSTEM_LIFETIME_LIMIT_CHECK_TIMEOUT,
    );
  }
  ...
  async _tick() {
    ...
    const links = await mapSqlActions.getLinksWithData(this.options.mapId, [
      "timeStatus",
      "created",
      "updated",
      "solarSystemSource",
    ...
    const systems = await mapSqlActions.getSystemsInfo(this.options.mapId);
      ...
      .reduce(
        (acc, sys) => ({
          ...
          [sys]:
            (acc[sys] || 0) +
            (links.filter(
              (x) => x.solarSystemSource === sys || x.solarSystemTarget === sys,
            )?.length || 0),
        }),
        {},
      );
    ...
    let needToRemove = systems
      ...
      .filter(
        (x) =>
          currentTime - +new Date(x.updatedTime) > SYSTEM_LIFETIME_LIMIT_MS,
      )
      .map((x) => x.id);

    if (needToRemove.length > 0) {
      const res =
        await mapSqlActions.countOnlineCharactersByLocations(needToRemove);

      needToRemove = needToRemove.filter(
        (sys) =>
          parseInt(
            res.find((x) => x.location === sys)?.character_count ?? "0",
          ) === 0,
      );
    }
  ...
  async getChain(chainId) {
    if (!this._chains[chainId]) {
      const chainInfo = await mapSqlActions.getLinkInfo(
        this.options.mapId,
        chainId,
      );
      if (!chainInfo) {
  ...
  async removeCharactersFromTracking(userId, charIds) {
    await Promise.all(
      charIds.map(this.removeCharacterFromTracking.bind(this, userId)),
    );
  }
  ...
  _createSystemObject(_systemId) {
    if (!exist(this._systems[_systemId])) {
      this._systems[_systemId] = new MapSolarSystem(
        this.options.mapId,
        _systemId,
      );
  ...
  _startObserverCharacter(userId, charId) {
    ...
    if (!hasCharacter) {
      ...
      this.characters[charId].on(
        "onlineChanged",
        this._onCharacterOnlineChanged.bind(this, charId),
      );
      this.characters[charId].on(
        "leaveSystem",
        this._onCharacterLeaveSystem.bind(this, charId),
      );
      this.characters[charId].on(
        "enterInSystem",
        this._onCharacterEnterInSystem.bind(this, charId),
      );
      this.characters[charId].on(
        "moveToSystem",
        this._onCharacterMoveToSystem.bind(this, charId),
      );
      this.characters[charId].on(
        "shipChanged",
        this._onCharacterShipChanged.bind(this, charId),
      );
      this.characters[charId].on(
        "drop",
        this._onCharacterDrop.bind(this, charId),
      );
  ...
  async _onCharacterDrop(characterId) {
    ...
    const userId = this._charactersOnUsers[characterId];
    if (userId) {
  ...
  async dropCharacter(charId) {
    ...
    const isOnline = this.characters[charId].isOnline();
    this.characters[charId].destructor();
    delete this.characters[charId];
    const currentSystemId = this._charactersOnSystem[charId];
  ...
  async _onCharacterLeaveSystem(characterId) {
    await this._characterLeaveSystem(
      characterId,
      this._charactersOnSystem[characterId],
    );
    this.subscribers.notifyCharacterOnlineRemoved(characterId);
  }
  ...
  async _onCharacterMoveToSystem(characterId, oldSystem, location) {
    ...
    this.subscribers.notifyCharacterOnlineUpdatedLocation(
      characterId,
      oldSystem,
      location,
    );
  ...
  async _addSystem(_oldSystem, solarSystemId, position) {
    let pos = position;
    this._createSystemObject(solarSystemId);
    const solarSystem = this._systems[solarSystemId];
    ...
    const result = await mapSqlActions.isSystemExistsAndVisible(
      this.options.mapId,
      solarSystemId,
    );
    if (result.exists && result.visible) {
      ...
      await mapSqlActions.changeSystemVisibility(
        this.options.mapId,
        solarSystemId,
        true,
      );

      const isOldVisible = await mapSqlActions.isSystemExistsAndVisible(
        this.options.mapId,
        _oldSystem,
      );
      ...
      const solarSystemInfo =
        await solarSystemSql.getSolarSystemInfo(solarSystemId);
  ...
  async _addLink(_sourceSystemId, _targetSystemId) {
    let chainInfo =
      this._chains[_sourceSystemId + "_" + _targetSystemId] ||
      this._chains[_targetSystemId + "_" + _sourceSystemId];
    if (!chainInfo) {
      chainInfo = this._chains[_sourceSystemId + "_" + _targetSystemId] = {
        promise: new CustomPromise(),
      };

      const chainData = await mapSqlActions.getLinkByEdges(
        this.options.mapId,
        _sourceSystemId,
        _targetSystemId,
      );
      if (!chainData) {
        const newChainId = _sourceSystemId + "_" + _targetSystemId;
        await mapSqlActions.addLink(
          this.options.mapId,
          newChainId,
          _sourceSystemId,
          _targetSystemId,
        );
        this._chains[newChainId].model = new MapChain(
          this.options.mapId,
          newChainId,
        );
        ...
        this._chains[chainInfo.id].model = new MapChain(
          this.options.mapId,
          chainInfo.id,
        );
      }
  ...
  async _linkPassage(_sourceSystemId, _targetSystemId, _characterId) {
    ...
    const link = await mapSqlActions.getLinkByEdges(
      this.options.mapId,
      _sourceSystemId,
      _targetSystemId,
    );
    await mapSqlActions.updateChainPassages(
      this.options.mapId,
      link.id,
      ++link.countOfPassage,
    );
  ...
  async _characterEnterToSystem(_characterId, solarSystemId) {
    const info = await solarSystemSql.getSolarSystemInfo(solarSystemId);
    if (info === null) {
      return;
    }
    ...
    const isAbleToEnter =
      !prohibitedSystemClasses.includes(parseInt(systemClass)) &&
  ...
  async _characterMoveToSystem(_characterId, _oldSystem, _newSystem) {
    ...
    const isJump = await core.sdeController.checkSystemJump(
      _oldSystem,
      _newSystem,
    );
    ...
    if (!isJump && isAbleToMove) {
      const isSystemExists = await this.systemExists(_oldSystem, true);
      if (isSystemExists) {
      ...
      const isDestSystemExists = await this.systemExists(_newSystem, true);
      const isOldSystemExists = await this.systemExists(_oldSystem, true);
  ...
  async addManual(solarSystemId, x, y) {
    this._createSystemObject(solarSystemId);
    const solarSystem = this._systems[solarSystemId];
    const result = await solarSystem.isSystemExistsAndVisible();
  ...
  async addHub(solarSystemId) {
    const hubs = await core.dbController.mapsDB.get(this.options.mapId, "hubs");

    const hasSystem = hubs.indexOf(solarSystemId.toString()) !== -1;
  ...
  async removeHub(solarSystemId) {
    ...
    const index = hubs.indexOf(solarSystemId.toString());
    if (index !== -1) {
  ...
  async getRoutesListForSolarSystemAdvanced(solarSystemId, hubs, settings) {
    ...
    if (!defaultSettings.avoidWormholes) {
      const mapChains = await this.getLinkPairsAdvanced(
      ...
      if (!defaultSettings.includeCruise) {
        chains = chains.filter((x) => {
          return (
            !core.cachedDBData.wormholeClassAIndexed[x.first] &&
    ...
    if (defaultSettings.avoidEdencom) {
      avoidanceList = avoidanceList.concat(
        core.cachedDBData.edencomSolarSystems,
      );
    }

    if (defaultSettings.avoidTriglavian) {
      avoidanceList = avoidanceList.concat(
        core.cachedDBData.triglavianSolarSystems,
      );
    }

    if (defaultSettings.avoidPochven) {
      avoidanceList = avoidanceList.concat(
        core.cachedDBData.pochvenSolarSystems,
      );
    }

    const out = [];

    const arrRoutes = await Promise.all(
      hubs.map((destination) =>
        this.loadRoute(
          destination,
          solarSystemId,
          defaultSettings.pathType,
          connections,
          avoidanceList,
        ),
      ),
    ...
    for (let a = 0; a < arrRoutes.length; a++) {
      const destination = hubs[a];
      const route = arrRoutes[a];

      const arrInfo = await Promise.all(
        route.systems.map((x) => getSolarSystemInfo(x, minimumRouteAttrs)),
      );
      const info = await getSystemInfo(this.options.mapId, destination, true);
      ...
      out.push({
        ...
        destination,
        destinationName:
          info?.userName || arrInfo[arrInfo.length - 1].solarSystemName,
      });
    }
  ...
  loadRoute(dest, origin, flag, connections, avoidanceList) {
    ...
    core.esiApi.routes(dest, origin, flag, connections, avoidanceList).then(
      (event) =>
        pr.resolve({
          hasConnection: event.length > 0,
          systems: event.length === 0 ? [dest] : event,
        }),
  ...
  async findPosition(_oldSystemId /*, _systemId*/) {
    ...
    if (_oldSystemId !== null) {
      const oldSystemPositionResult = await mapSqlActions.getSystemPosition(
        this.options.mapId,
        _oldSystemId,
      );

      if (exist(oldSystemPositionResult)) {
        const oldPosition = oldSystemPositionResult.position;
  ...
  async updateSystemsPosition(_systemsPosition) {
    await Promise.all(
      _systemsPosition.map((x) =>
        this._systems[x.id].updatePositions(x.x, x.y),
      ),
    );
  }
  ...
  async getLinkPairsAdvanced(includeFrig, includeEol, includeMassCrit) {
    return await mapSqlActions.getLinkPairsAdvanced(
      this.options.mapId,
      includeFrig,
      includeEol,
      includeMassCrit,
  ...
  async systemExists(_systemId, checkVisible) {
    return await mapSqlActions.systemExists(
      this.options.mapId,
      _systemId,
      checkVisible,
    );
  ...
  async linkRemove(chainId) {
    ...
    const chainInfo = await mapSqlActions.linkRemove(
      this.options.mapId,
      chainId,
    );
    let info;
  ...
  async systemRemove(_systemId) {
    ...
    const affectedLinks = await mapSqlActions.getLinksBySystem(
      this.options.mapId,
      _systemId,
    );
    ...
    await Promise.all(
      this._systems[_systemId]?.onlineCharacters?.map((x) => {
        delete this._charactersOnSystem[x];
        return mapSqlActions.removeCharacterFromSystem(
          this.options.mapId,
          _systemId,
          x,
        );
  ...
  async subscribeAllowedCharacters(connectionId, responseId, userId) {
    const characters =
      await core.mapController.getTrackingCharactersForMapByUser(
        this.options.mapId,
        userId,
      );
    ...
    if (this._users[userId].subscribersCount() === 0) {
      const trackedCharacters = characters.reduce((acc, charId) => {
        if (!this.characters[charId]) {
          return acc;
        }
  ...
  async unsubscribeAllowedCharacters(connectionId, responseId, userId) {
    this._users[userId].characters.map(
      ({ charId }) => delete this._charactersOnUsers[charId],
    );
...
const removeIntersection = function (pairsArr) {
  const checkObj = Object.create(null);
  const out = [];

  pairsArr.map((x) => {
    if (
      !checkObj[x.first + "_" + x.second] &&
      !checkObj[x.second + "_" + x.first]
    ) {
