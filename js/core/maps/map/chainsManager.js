...
class ChainsManager extends Emitter {
  ...
  async _tick() {
    const links = await mapSqlActions.getLinksWithData(this._mapId, [
      "timeStatus",
      "created",
      "updated",
    ]);
    ...
    for (let a = 0; a < links.length; a++) {
      const lastUpdate = +new Date(links[a].updated);

      if (links[a].timeStatus === 0) {
        const isEOL =
          currentTime > lastUpdate + LINK_EOL &&
          currentTime < lastUpdate + LINK_LIFETIME;
        const isExpire = currentTime > lastUpdate + LINK_LIFETIME;
        ...
        const isExpire = currentTime > lastUpdate + LINK_EOL_DELTA;
        if (isExpire)
          changedChains.push({ id: links[a].id, newState: "Expired" });
      }
    }

    if (changedChains.length > 0) this.emit("changedChains", changedChains);
