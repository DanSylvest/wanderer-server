...
class MapCharacter extends Emitter {
  ...
  init() {
    ...
    this.onlineAttr = core.charactersController
      .get(this.characterId)
      .get("online");
    this.onlineSubscribeId = this.onlineAttr.on(
      "change",
      this._onOnlineChange.bind(this),
    );
  ...
  watchToShipType() {
    ...
    this.shipSubscribeId = this.shipAttr.on(
      "change",
      this._onShipChange.bind(this),
    );
  }

  watchToLocation() {
    this.locationAttr = core.charactersController
      .get(this.characterId)
      .get("location");
    this.locationSubscribeId = this.locationAttr.on(
      "change",
  ...
  _onLocationChange(_location) {
    const location = _location.toString();
    ...
    switch (this.state) {
      ...
      case ST_READY:
        if (this.locationValue !== location) {
          const oldSystem = this.locationValue;
          this.locationValue = location;
