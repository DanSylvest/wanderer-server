...
class MapUser extends Emitter {
  ...
  unsubscribeAllowedCharacters(connectionId, responseId) {
    if (this._allowedCharactersSubscriber) {
      this._allowedCharactersSubscriber.removeSubscriber(
        connectionId,
        responseId,
      );
    }
  ...
  onlineChanged(charId, online) {
    const obj = this.characters.searchByObjectKey("charId", charId);

    if (!obj) {
      log(
        log.TRACE,
        `Object for char [${charId}] - was not found with online status - ${online}`,
      );
    }
  ...
  subscribersCount() {
    return this._allowedCharactersSubscriber
      ? this._allowedCharactersSubscriber.subscribersCount()
      : 0;
  }
}
