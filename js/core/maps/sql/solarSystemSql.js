 ...
 */
const getSolarSystemInfo = async function (solarSystemId, attrs = core.dbController.solarSystemsTable.attributes()) {
  ...
  const constellationInfo = await core.esiApi.universe.constellation(systemInfo.constellationId);
  const regionInfo = await core.esiApi.universe.region(constellationInfo.regionId);
  ...
  const security = (parseInt(securityStatus) === -0.99 ? -1.0 : parseInt(securityStatus)).toFixed(1);
  const out = {
