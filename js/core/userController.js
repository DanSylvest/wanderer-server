...
class UserController extends Emitter {
  ...
  async registerUserByEveSSO(code) {
    const data = await this._verifyAuthCode(code);

    const existsInUsers = await core.dbController.userDB.existsByCondition([
      { name: "type", operator: "=", value: 1 },
      { name: "id", operator: "=", value: data.userData.CharacterID },
    ]);
    const existsInCharacters =
      await core.dbController.charactersDB.existsByCondition([
        { name: "id", operator: "=", value: data.userData.CharacterID },
      ]);

    if (!existsInUsers && existsInCharacters) {
      const userId = await this.getUserByCharacter(
        data.userData.CharacterID.toString(),
      );
      ...
      await this._boundUserAndCharacter(
        data.userData.CharacterID,
        data.userData.CharacterID,
      );
      return await core.tokenController.generateToken(
        data.userData.CharacterID,
      );
    } else if (existsInUsers && !existsInCharacters) {
  ...
  async _boundUserAndCharacter(_userName, _characterId) {
    ...
    if (result.length !== 0) {
      throw {
        message: `Character ${_characterId} already attached to user ${_userName}`,
      };
    }
  ...
  async updateUserOnlineStatus(connectionId, token) {
    ...
    const isOnline = this.getUserOnline(userId);
  ...
  async updateUserOfflineStatus(connectionId, token) {
    ...
    if (this.userAtConnection.has(userId, connectionId)) {
      ...
      await core.mapController.dropCharsFromMapsByUserAndConnection(
        userId,
        connectionId,
      );

      const count = this.userAtConnection.delete(userId, connectionId);
      if (isOnline && count === 0) {
