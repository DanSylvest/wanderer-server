...
class MultiObject {
  ...
  delete(key, val) {
    if (!exist(val)) delete this._items[key];

    const index = this._items[key].indexOf(val);
  ...
  has(key, val) {
    if (!exist(val) && !exist(this._items[key])) return false;

    if (
      (exist(val) && !exist(this._items[key])) ||
      this._items[key].indexOf(val) === -1
    ) {
      return false;
    }
