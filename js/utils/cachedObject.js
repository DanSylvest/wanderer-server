...
class CachedObject {
    ...
    this.options = extend(
      {
        ...
        expireTime: 10000,
        lifetime: 10000,
        remoteGetterAsync: (resolve) => {
          resolve({
            exists: true,
            data: 0,
          });
        },
        dbGetterAsync: (resolve) => {
  ...
  _checkDB() {
    ...
    pr.native.then(
      ...
      (error) => {
        // Если что-то пошло не так прям в базе данных...
        // но наверно до такого нельзя допускать
        throw { error, message: "Error on get from database" };
      },
    );
  }
   ...
   */
  _updateSecondLevelCache(data) {
    const expires = +new Date() + this.options.expireTime;
    ...
    pr.native.then(
      ...
      (error) => {
        throw { error, message: "Error on set to database" };
      },
    );
  }
