...
const Provider = classCreator("Provider", Emitter, {
  ...
  start() {
    this.options.showLogs &&
      log(
        log.DEBUG,
        printf("Provider [%s] has been started", this.options.name),
      );
  ...
  stop() {
    this.options.showLogs &&
      log(
        log.DEBUG,
        printf("Provider [%s] has been stopped", this.options.name),
      );
    this._tid !== -1 && clearTimeout(this._tid);
    this._tid = -1;
    this._isStarted = false;
  ...
  _tick() {
    this.options.showLogs &&
      log(log.DEBUG, printf("Provider [%s] tick", this.options.name));
    if (this.options.accessToken) {
      this.options.accessToken().then(
        (_token) => {
          this.options.showLogs &&
            log(
              log.DEBUG,
              printf(
                "Provider [%s] loading with access token [%s]",
                this.options.name,
                _token,
              ),
            );
      ...
      log(
        log.DEBUG,
        printf("Provider [%s] loading without access token", this.options.name),
      );
  ...
  _triggerTimeout() {
    this.options.showLogs &&
      log(
        log.DEBUG,
        printf(
          "Provider [%s] timeout [%sms] started",
          this.options.name,
    ...
    if (this._fastRequest) {
      ...
      this._tid = setTimeout(
        this._triggerTimeoutEnd.bind(this),
        this.options.timeout,
      );
    }
  },
  ...
  _notify(_value) {
    ...
    this.options.showLogs &&
      log(log.DEBUG, printf("Provider [%s] notify", this.options.name));
