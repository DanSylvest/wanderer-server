...
const searchDumpFile = function (_path) {
  const list = fs.readdirSync(_path.toString());

  if (list.length === 0) throw "EVE SDE Dump file not exists";
...
const installSDE = async function (client, conString) {
  ...
  const dumpFile = searchDumpFile(dirPath["+"]("eveData/dump"));
  ...
  log(
    log.INFO,
    `Restore dump database ${dumpFile} for ${EVE_SDE_DB_NAME}. ~(2-3 min)`,
  );
  await execProcess(
    printf(
      "pg_restore --no-privileges --dbname=%s/%s %s",
      conString,
      EVE_SDE_DB_NAME,
      dirPath["+"]("eveData/dump")["+"](dumpFile).toString(),
    ),
  );
